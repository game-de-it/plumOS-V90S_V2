...
EOF
  ...
  {
    printf 'artifact_name=nextcommander\n'
    printf 'artifact_type=application\n'
    printf 'target=powkiddy-v90s-aarch64\n'
    printf 'version_or_git_ref=%s\n' "${NEXTCOMMANDER_REF}"
    printf 'source_url_or_input_path=%s\n' "${NEXTCOMMANDER_REPO}"
    printf 'patches=nextcommander-v90s.patch\n'
  ...
  } >"${TARGET_DIR}/nextcommander.manifest"
