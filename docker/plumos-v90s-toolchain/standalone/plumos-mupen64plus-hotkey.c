...
static int open_adc_gamepad(char *device_path, size_t device_path_size) {
  char name_path[128];
  char name[128];
  int index;
...
int main(int argc, char **argv) {
  ...
  input_fd = open_adc_gamepad(device_path, sizeof(device_path));
  if (input_fd < 0) {
    fprintf(stderr, "mupen64plus-hotkey: adc_gamepad input not found\n");
    return 4;
  }
  ...
  fprintf(stderr, "mupen64plus-hotkey: monitoring %s BTN_MODE\n", device_path);
  ...
  while (!stop_requested && target_matches(target_pid, expected_exe)) {
    ...
    for (index = 0; index < event_count; index++) {
      if (events[index].type == EV_KEY && events[index].code == BTN_MODE &&
