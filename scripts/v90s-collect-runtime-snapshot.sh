...
run_sh "interrupt-rate-5s" 'sample() { key="$1"; awk -v key="$key" "$0 ~ key { s=0; for (i=2; i<=NF; i++) if (\$i ~ /^[0-9]+$/) s+=\$i; print s; found=1 } END { if (!found) print \"\" }" /proc/interrupts 2>/dev/null; }; d0=$(sample "dispaly|display"); p0=$(sample "pvrsrvkm|pvr|rgx"); t0=$(date +%s); sleep 5; d1=$(sample "dispaly|display"); p1=$(sample "pvrsrvkm|pvr|rgx"); t1=$(date +%s); dt=$((t1 - t0)); echo "display_before=$d0 display_after=$d1"; echo "pvr_before=$p0 pvr_after=$p1"; if [ -n "$d0" ] && [ -n "$d1" ] && [ "$dt" -gt 0 ]; then awk -v a="$d0" -v b="$d1" -v dt="$dt" "BEGIN { printf \"display_delta=%d display_hz=%.5f\\n\", b-a, (b-a)/dt }"; fi; if [ -n "$p0" ] && [ -n "$p1" ] && [ "$dt" -gt 0 ]; then awk -v a="$p0" -v b="$p1" -v dt="$dt" "BEGIN { printf \"pvr_delta=%d pvr_hz=%.5f\\n\", b-a, (b-a)/dt }"; fi'
...
run_sh "retroarch-configs" 'for f in /mnt/share/retroarch/retroarch-v90s.cfg /tmp/retroarch-v90s.cfg /storage/.config/retroarch/retroarch.cfg /userdata/system/configs/retroarch/retroarch.cfg /usr/share/knulli/datainit/system/configs/retroarch/retroarch.cfg /etc/retroarch.cfg /root/.config/retroarch/retroarch.cfg; do [ -f "$f" ] && echo "--- $f" && grep -n -E "^(video_driver|video_context_driver|video_refresh_rate|video_vsync|vrr_runloop_enable|video_threaded|video_hard_sync|video_swap_interval|audio_driver|audio_device|audio_sync|audio_latency|audio_rate_control|audio_rate_control_delta|audio_max_timing_skew|audio_out_rate|input_driver|input_joypad_driver|fps_show)" "$f"; done'
