...
run_sh "retroarch-version" 'ra_env="/usr/lib/powervr:/usr/local/lib/plumos-sdl2-powervr:/usr/local/lib/plumos-sdl2-mali:/usr/lib/aarch64-linux-gnu:/usr/lib:$LD_LIBRARY_PATH"; for b in retroarch /usr/bin/retroarch /usr/local/bin/retroarch /tmp/retroarch-knulli; do if command -v "$b" >/dev/null 2>&1 || [ -x "$b" ]; then echo "--- $b plain"; "$b" --version 2>&1; echo "--- $b pvr-env"; env LD_LIBRARY_PATH="$ra_env" "$b" --version 2>&1; fi; done'
run_sh "retroarch-features" 'ra_env="/usr/lib/powervr:/usr/local/lib/plumos-sdl2-powervr:/usr/local/lib/plumos-sdl2-mali:/usr/lib/aarch64-linux-gnu:/usr/lib:$LD_LIBRARY_PATH"; for b in retroarch /usr/bin/retroarch /usr/local/bin/retroarch /tmp/retroarch-knulli; do if command -v "$b" >/dev/null 2>&1 || [ -x "$b" ]; then echo "--- $b plain"; "$b" --features 2>&1; echo "--- $b pvr-env"; env LD_LIBRARY_PATH="$ra_env" "$b" --features 2>&1; fi; done'
