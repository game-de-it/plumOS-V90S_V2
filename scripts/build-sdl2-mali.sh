...
usage() {
    cat <<'USAGE'
Usage:
  build-sdl2-mali.sh [options]

Options:
  --version VERSION    SDL2 release version, default 2.30.6
  --out-dir PATH      output directory, default output/sdl2-mali
...
if [ "$(uname -s)" != "Linux" ]; then
    printf 'error: this script builds Linux arm64 binaries; run it through scripts/run-assembly-tools.sh\n' >&2
    exit 1
fi
...
work_dir="${TMPDIR:-/tmp}/plumos-v90s-sdl2-mali.$$"
repo_root="$(pwd)"
rm -rf "$work_dir"
mkdir -p "$work_dir"
...
filtered_patch="$work_dir/sdl2-mali-filtered.patch"
...
rm -rf "$out_dir"
mkdir -p "$out_dir/usr/local/lib/plumos-sdl2-mali" "$out_dir/usr/local/bin"
cp -a "$work_dir/stage/usr/lib/libSDL2-2.0.so"* "$out_dir/usr/local/lib/plumos-sdl2-mali/"
cp -a "$work_dir/stage/usr/lib/libSDL2.so" "$out_dir/usr/local/lib/plumos-sdl2-mali/"
...
gcc \
    ...
    -Wl,-rpath,/usr/local/lib/plumos-sdl2-mali \
    -lSDL2 \
    -o "$out_dir/usr/local/bin/v90s-sdl2-video-probe"

strip --strip-unneeded "$out_dir/usr/local/lib/plumos-sdl2-mali/libSDL2-2.0.so.0.3000.6" 2>/dev/null || true
...

{
    ...
    strings "$out_dir/usr/local/lib/plumos-sdl2-mali/libSDL2-2.0.so.0" | grep -E '^(mali|x11|wayland|kmsdrm|dummy|offscreen)$' | sort -u | tr '\n' ' '
    printf '\n'
