#!/usr/bin/env python3
"""Transactional Runtime and A/B System updater for plumOS V90S."""
...
BOOT_ROOT = Path(os.environ.get("PLUMOS_BOOT_ROOT", "/mnt/plumos-boot"))
...
DEVICE_ID = "powkiddy-v90s"
ARCHITECTURE = "aarch64"
VENDOR_RUNTIME = "v90s-stockos-r1"
METADATA_LAST = {"VERSION", "manifest.json", "checksums.sha256"}
MANAGED_ROOTS = {
    "adb", "apps", "audio-router", "bin", "cores", "factory-defaults",
    "fonts", "frontend", "gnu", "info", "lib", "licenses", "media",
...
def validate_manifest(manifest: dict[str, Any], archive_names: set[str]) -> None:
    ...
    if manifest.get("device_id") != DEVICE_ID or manifest.get("architecture") != ARCHITECTURE:
        raise UpdateError("package target does not match V90S aarch64")
...
def current_compatibility(manifest: dict[str, Any]) -> None:
    ...
    try:
        if manifest["package_type"] == "runtime":
            ...
            compatible = int(manifest["runtime_abi_min"]) <= runtime_abi <= int(manifest["runtime_abi_max"])
            message = "runtime ABI is outside the system package range"
...
def scan_packages() -> list[dict[str, Any]]:
    ...
    if not inbox.is_dir():
        return results
    for path in sorted(inbox.glob("plumos-v90s-*.tar.gz")):
        try:
            item = inspect(path)
...
def apply_system(package: Path, manifest: dict[str, Any], manifest_bytes: bytes, signature: bytes) -> int:
    ...
    active = read_text(SYSTEM_ACTIVE, read_text(BOOT_ROOT / "System/active-slot", "a"))
    if active not in {"a", "b"}:
        active = "a"
    inactive = "b" if active == "a" else "a"
    system_dir = BOOT_ROOT / "System"
