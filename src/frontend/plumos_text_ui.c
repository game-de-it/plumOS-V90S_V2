...
static int plumos_root_from_systems_path(const char *systems_path,
                                         char *out, size_t out_size) {
  static const char suffix[] = "/config/frontend/systems.json";
  size_t root_len;

  if (!systems_path || !out || out_size == 0 || !path_has_suffix(systems_path, suffix)) {
    return 0;
  }
  root_len = strlen(systems_path) - strlen(suffix);
  if (root_len == 0 || root_len >= out_size) {
    return 0;
  }
  memcpy(out, systems_path, root_len);
  out[root_len] = '\0';
  return 1;
}

static int onion_compatible_emulators_required_for_systems_path(const char *systems_path) {
  char plumos_root[PATH_MAX];
  char flag_path[PATH_MAX];

  if (!plumos_root_from_systems_path(systems_path, plumos_root, sizeof(plumos_root))) {
    return 0;
  }
...
static int picoarch_compatible_adapter_ready_for_systems_path(const char *systems_path) {
  char plumos_root[PATH_MAX];
  char guard_path[PATH_MAX];
  char adapter_path[PATH_MAX];
  char delegated_path[PATH_MAX];

  if (!plumos_root_from_systems_path(systems_path, plumos_root, sizeof(plumos_root))) {
    return 0;
  }
  return join_path(guard_path, sizeof(guard_path), plumos_root, "bin/plumos-picoarch-launch") &&
         join_path(adapter_path, sizeof(adapter_path), plumos_root,
                   "bin/plumos-picoarch-onion-launch") &&
         join_path(delegated_path, sizeof(delegated_path), plumos_root,
                   "bin/plumos-picoarch-launch.noncompat") &&
         file_exists(guard_path) && file_exists(adapter_path) && file_exists(delegated_path);
}
...
static int load_core_system_def(const char *path, const char *system_id,
  ...
  while (1) {
    ...
    if (!onion_compatible_emulators_required_for_systems_path(path) ||
        picoarch_compatible_adapter_ready_for_systems_path(path)) {
...
static void apply_picoarch_adapter_cpu_defaults(const char *plumos_root,
  ...
  if (cpu_cores && cpu_cores_source &&
      strcmp(cpu_cores_source, "plumOS default") == 0) {
    *cpu_cores = 0;
  }
}
...
static void usage(const char *argv0) {
  ...
  printf("\n");
  printf("Environment:\n");
  printf("  PLUMOS_SDCARD_ROOT  Default: /mnt/plumos\n");
  printf("  PLUMOS_ROOT         Default: $PLUMOS_SDCARD_ROOT\n");
...
int main(int argc, char **argv) {
  ...
  if (plumos_root_env && plumos_root_env[0]) {
    ...
    if (!copy_string(plumos_root, sizeof(plumos_root), sdcard_root)) {
    ...
    for (i = option_start; i < argc; i++) {
      if (strcmp(argv[i], "--set") == 0 && i + 1 < argc) {
        ...
        if (!parse_cpu_cores(argv[++i], &set_cpu_cores)) {
          fprintf(stderr, "error: --cores expects 2\n");
          return 2;
        }
        cores_seen = 1;
